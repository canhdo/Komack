...
data$HEdu <-ifelse(data$Edu >=3,1,0) # High Education
...
data$HJob <- ifelse(data$Job <6,1,0)
...
#data$BTPP1 <- ifelse(data$TPP1==1,1,0) # business
#data$BTPP2 <- ifelse(data$TPP1==2,1,0) # backhome
#data$BTPP3 <- ifelse(data$TPP1==3,1,0) #holiday
#data$BTPP4 <- ifelse(data$TPP1==4,1,0) #Visiting
#data$BTPP4 <- ifelse(data$TPP1==5,1,0) #education
#data$BTPP4 <- ifelse(data$TPP1==6,1,0) #other


#data$TPP <- ifelse(data$TPP1>1,1,0) # business purpose
...
data$TPP <- ifelse(data$TPP==1,1,0) # business purpose
...
data$Cost_MT   <- data$Cost_MT/1000
data$Cost_Car  <- data$Cost_Car/1000
data$Cost_Taxi <- data$Cost_Taxi/1000
data$Cost_Bus  <- data$Cost_Bus/1000
data$Cost_VCAT <- data$Cost_VCAT/1000


# Dummies parking/1000

data$Park_MT <- data$Park_MT/1000
data$Park_Car <- data$Park_Car/1000
data$Park_Taxi <- data$Park_Taxi/1000
data$Park_Bus <- data$Park_Bus/1000
data$Park_VCAT <- data$Park_VCAT/1000
### Correlation matrix ####
...
{ #Begin function
  ...
  Vc   <- 0    + #data$Dependency*DepenC + #data$Rain*RainC +
     data$Cost_Car*Cost #+ data$Park_Car*Parking + data$Vehtime_Car*Vehtime  + data$Delay_Car*Delay #+ data$Safe_Car*Safety
  
  Vm   <- Cm   + #data$Lao*LaoM + Male*MaleM*data$Lao  +  data$Age6*Age60M   + #data$HJob*HJobM  +
    #data$HEdu*HEduM  + data$LUG*LugM   + data$TPP*BusiTripM + data$Accom*AccomM +
    #data$Dependency*DepenM + data$Hcar*HcarM + data$Inc*HIncM +
    data$Cost_MT*Cost  #+ data$Park_MT*Parking + data$Vehtime_MT*Vehtime #+ data$Safe_MT*Safety
  
  Vt   <- Ct   + #data$Lao*LaoT + Male*MaleT*data$Lao   +  data$Age6*Age60T  + #data$HJob*HJobT  +
    #data$HEdu*HEduT + data$LUG*LugT  + data$Accom*AccomT + data$TPP*BusiTripT + #data$Rain*RainT+
    #0   + data$Dependency*DepenT + data$Hcar*HcarT + data$Inc*HIncT +
    data$Cost_Taxi*Cost #+ data$Vehtime_Taxi*Vehtime + data$Delay_Taxi*Delay #+ data$Safe_Taxi*Safety
  
  Vb   <- Cb   + #data$Lao*LaoB + Male*MaleB*data$Lao    +  data$Age6*Age60B + #data$HJob*HJobB  +
    #data$HEdu*HEduB + data$LUG*LugB  + data$Accom*AccomB + data$TPP*BusiTripB +#data$Rain*RainB  +
    #0 + data$Hcar*HcarB  + data$Inc*HIncB +
    data$Cost_Bus*Cost  #+ data$Vehtime_Taxi*Vehtime + data$Delay_Bus*Delay + data$Wtime_Bus*Waiting + data$Acctime_Bus*Access  #+ data$Safe_Bus*Safety #+ data$Trans_Bus*Transfer
  
  Vv   <- Cv   +# data$Lao*LaoV + Male*MaleV*data$Lao    +  data$Age6*Age60V + #data$HJob*HJobV   +
    #data$HEdu*HEduV  + data$LUG*LugV  + data$Accom*AccomV + data$TPP*BusiTripV +
    #0    + data$Dependency*DepenV  +  data$Hcar*HcarV  +  data$Inc*HIncV  + #+ data$Rain*RainV # data$Inc*HIncV  +
    data$Cost_VCAT*Cost #+ data$Vehtime_VCAT*Vehtime + data$Wtime_VCAT*Waiting  + data$Acctime_VCAT*Access #+ data$Safe_VCAT*Safety #+ data$Trans_VCAT*Transfer
